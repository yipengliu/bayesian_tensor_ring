function [B,LB] = TR_VBI_laplace(Y, O, init, maxRank , maxiters, tol)  % tensor ring with Bayesian variational inference
%  INPUTS
%     Y              - Input tensor
...
for it=1:maxiters
    ...
    for n=1:N
        [gammas{n},d_gammaN{n}]=update_gamma_laplace(d_gamma0{n},G([n:N, 1:n-1]),gammas([n:N, 1:n-1]),V([n:N, 1:n-1]));
    end
...
%     LB(it) = temp1 + temp2 + temp3 + temp4 + temp5 + temp6 ;
    ...
    for n=1:N-1
      [G{n},G{n+1},V{n},V{n+1},gammas{n},r]=reduce_dimension(G([n:N, 1:n-1]),V([n:N, 1:n-1]),gammas([n:N, 1:n-1]),it);
        R(n)=r;
    end
     [G{N},G{1},V{N},V{1},gammas{N},r]=reduce_dimension(G([N, 1:N-1]),V([N, 1:N-1]),gammas([N, 1:N-1]),it);
         R(N)=r;
    %% plot 
     % Notice:the number of gammas is corresponding to  the data dimension
        ...
        subplot(3,4,5); bar(gammas{3}); title('Posterior mean of \lambda_{3}'); xlabel('Latent components'); ylabel(''); axis tight;
        subplot(3,4,6); bar(gammas{4}); title('Posterior mean of \lambda_{4}'); xlabel('Latent components'); ylabel(''); axis tight;
        subplot(3,4,7); bar(gammas{3}); title('Posterior mean of \lambda_{3}'); xlabel('Latent components'); ylabel(''); axis tight;
        subplot(3,4,8); bar(gammas{4}); title('Posterior mean of \lambda_{4}'); xlabel('Latent components'); ylabel(''); axis tight;
        subplot(3,4,9); bar(gammas{4}); title('Posterior mean of \lambda_{4}'); xlabel('Latent components'); ylabel(''); axis tight;
