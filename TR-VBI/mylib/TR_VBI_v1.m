function [model] = TR_VBI_v1(Y, O, init, maxRank , maxiters, tol)  % tensor ring with Bayesian variational inference
%  INPUTS
%     Y              - Input tensor
...
for it=1:maxiters
    %% Update factor cores
     for n=1:N
     [G{n},GSigma{n},V{n}]=update_factor_v1(TensPermute(Y, n),TensPermute(O, n), G([n:N, 1:n-1]),V([n:N, 1:n-1]),gammas([n:N, 1:n-1]),tau,I([n:N, 1:n-1]));
     end
         %% update noise tau
   B=Ui2U(G);
   ...
   err(it) = Y(:)'*Y(:) - 2*Y(:)'*B(:) + EX2;
    ...
    for n=1:N
        [gammas{n},c_gammaN{n},d_gammaN{n}]=update_gamma_v1(c_gamma0{n},d_gamma0{n},G([n:N, 1:n-1]),I([n:N, 1:n-1]),V([n:N, 1:n-1]));
    end
   ...
   deta=sqrt(err(it))/norm(Y(:),'fro')/sqrt(N);
    ...
    for n=1:N-1
      [G{n},G{n+1},V{n},V{n+1},gammas{n},r,I{n}]=reduce_dimension_v11(G([n:N, 1:n-1]),V([n:N, 1:n-1]),gammas([n:N, 1:n-1]),deta,I([n:N, 1:n-1]));
        R(n)=r;
    end
     [G{N},G{1},V{N},V{1},gammas{N},r,I{N}]=reduce_dimension_v11(G([N, 1:N-1]),V([N, 1:N-1]),gammas([N, 1:N-1]),deta,I([N, 1:N-1]));
         R(N)=r;
