function [model] = TR_VBI_v2(Y, O, init, maxRank , maxiters, tol)  % tensor ring with Bayesian variational inference
%  INPUTS
%     Y              - Input tensor
...
for it=1:maxiters
    %% Update factor cores
     ...
     [G{n},GSigma{n},V{n}]=update_factor(TensPermute(Y, n),TensPermute(O, n), G([n:N, 1:n-1]),V([n:N, 1:n-1]),gammas([n:N, 1:n-1]),tau);
      ...
      %% Update hyperparameters gamma  Laplace distribution
    for n=1:N
        [gammas{n}]=update_gamma_v2(c_gamma{n},G([n:N, 1:n-1]),gammas([n:N, 1:n-1]),GSigma([n:N, 1:n-1]));
    end
    %% Update hyperparameters c_gamma
    for n=1:N
        c_gamma{n}=(length(gammas{n})-1)/(0.5*sum(1/gammas{n}));  
    end
